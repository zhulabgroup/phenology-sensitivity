...
create_plot <- function(npndata,name) {
  npndata %>% group_by(species_id) %>%
    summarise(obs_count = n()) %>%
    filter(obs_count == max(obs_count)) %>%
    slice(1) %>%
    inner_join(npndata, by = "species_id") %>% 
    ggplot() +
    geom_point(aes(x = x, y = y), col = "blue", pch = 16) +
    geom_point(aes(x = x, y = y_fit), col = "red", pch = 16) +
    ggtitle(paste(name,
      species_code[species_code$species_id == npndata$species_id[1], "common_name"],
      "Coefficient:",
      sprintf("%.3f", npndata$coefficient[[1]]$cor),
      "P_value:",
      sprintf("%.3f", npndata$coefficient[[1]]$p.value),
      sep = " "
    )
      ) +
...
site_gg <- map2(npndata, names(npndata), ~ create_plot(.x, .y))
