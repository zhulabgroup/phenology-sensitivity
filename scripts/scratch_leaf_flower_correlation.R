...
##### check which combination has the most points: 1,7; 2,7 #########
# wide_table <- npn_simple %>%
#   pivot_wider(names_from = pheno_class_id, values_from = first_yes_doy)
# 
# pairs(wide_table[,c(5:10)])

leaf <-npn_simple %>% filter(pheno_class_id %in% c(1, 2, 3))

flower <-npn_simple %>% filter(pheno_class_id %in% c(6, 7, 8))

temp <- inner_join(leaf, flower, by = c("individual_id", "first_yes_year", "species_id", "dataset_id") )

table(temp$pheno_class_id.x,temp$pheno_class_id.y)

#####

leafing <- npn_simple %>% filter(pheno_class_id == 2)
