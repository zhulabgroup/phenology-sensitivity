...
records_plot <- ggplot(count_data) + # "Field observation" = "#0751f1", "Herbarium" = "#582800"
  geom_line(aes(x = year, y = count, color = dataset)) +
    scale_color_manual(
  ...
  theme(
  legend.position = c(0.5, 1),     # x, y coordinates (center-top)
  legend.justification = c(0.5, 1),# align center of legend to top-center of plot
  legend.box.just = "center",     # optional, keeps alignment centered
    legend.direction = "horizontal",     # Make legend items go horizontally
  legend.background = element_rect(fill = "white", color = NA), # optional, for visibility
...
tem_sen <- ggplot(data = black) +
  ...
  annotate("text", x = 0.9, y = -0.1, label = "Warm year", size = 4, vjust = 0.2) +
...
byspecies_scatter <- by_species %>%
  ggplot(aes(x = norm_estimate, y = anom_estimate, color = dataset)) +
  geom_point(alpha = 0.7) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "grey50") +
  labs(
    x = "Spatial Sensitivity (days/°C)",
    y = "Temporal Sensitivity (days/°C)",
    color = "Dataset"  # Shared legend title
  ) +
  geom_xsidedensity(aes(y = after_stat(density), fill = dataset), alpha = 0.5) +
  geom_ysidedensity(aes(x = after_stat(density), fill = dataset), alpha = 0.5) +
  scale_color_manual(
    values = c("npn" = "#0751f1", "herb" = "#582800"),
    labels = c("npn" = "Field observation", "herb" = "Herbarium")
  ) +
  scale_fill_manual(
    values = c("npn" = "#0751f1", "herb" = "#582800"),
    labels = c("npn" = "Field observation", "herb" = "Herbarium")  # Add labels here
  ) +
...
# Table 1
```{r}
herb_table <- by_species_wtaxa_combined %>% 
  filter(dataset=="herb") %>% 
  group_by(taxa, sensitivity_type) %>% 
  summarise(
    count = n(),
    equal = sum(equal),
    neg = sum(conf_high_combined < 0),
    .groups = "drop"
  )
...
# Table 2
```{r}
npn_table <- by_species_wtaxa_combined %>% 
  filter(dataset=="npn", sensitivity_type == "spatial") %>% 
  group_by(taxa) %>% 
  summarise(
    count = n(),
    equal = sum(equal),
