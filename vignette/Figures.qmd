...
normality_plot <- red_maple %>% 
  ...
  geom_smooth(aes(group = dataset), method = "lm", se = TRUE, size = 1, color = "red") +
  labs(
    title = " ",
...
anomaly_plot <- red_maple %>% 
  ...
  geom_point(alpha = 0.5) +
  geom_smooth(aes(group = dataset), method = "lm", se = TRUE, size = 1, color = "purple") +
  labs(
    title = " ",
...
```{r}
red <- data.frame(x = c(0, 3), y = c(3, 0))/3
purple_counter  <- data.frame(x = c(1, 2), y = c(3, 0))/3
purple_co  <- data.frame(x = c(0, 3), y = c(2, 1))/3
```

```{r fig.width=4, fig.height=3}
spa_sen <- ggplot(data = red) +
  # geom_point(aes(x = x, y = y)) +
  geom_line(mapping = aes(x = x, y = y), color = "red", size = 1) +
  theme(
...
```{r fig.width=4, fig.height=3}
tem_sen <- ggplot(data = red) +
  # geom_point(aes(x = x, y = y)) +
  geom_line(mapping = aes(x = x, y = y), color = "purple", size = 1) +
  theme(
    axis.text.x = element_blank(),
    axis.text.y = element_blank(),
    axis.ticks = element_blank(),
...
co_gradient <- ggplot(mapping = aes(x = x, y = y)) +
      geom_line(data = red, color = "red", size = 1) +
geom_line(data = purple_co, color = "purple", size = 1) +
  ...
  labs(
    ...
    title = "Plasticity + Co-gradient adaptation"
  ) +
  ...
  geom_curve(
    aes(x = 1, y = 0.33, xend = 0.95, yend = 0.08),
    curvature = -0.1,
...
no_adaptation <- ggplot() +
  geom_line(data = red, aes(x = x, y = y), color = "purple", size = 1) +
    geom_line(data = red, aes(x = x, y = y+0.04), color = "red", size = 1) +
  # geom_hline(yintercept = 0.4, linetype = "dashed") +
  # geom_vline(xintercept = 0.6, linetype = "dashed") +
  labs(
    # x = expression(t[observe]),
...
counter_gradient <- ggplot(mapping = aes(x = x, y = y)) +
  geom_line(data = red, color = "red", size = 1) +
  geom_line(data = purple_counter, color = "purple", size = 1) +
  labs(
    x = "Temperature observed",
    y = "Flowering day observed",
    title = "Plasticity + Counter-gradient adaptation"
  ) +
  geom_text(aes(x = 0.9, y = 0.9, label = "−"), size = 10, color = "black") +
  geom_curve(
    aes(x = 0.67, y = 0, xend = 0.77, yend = 0.2),
    curvature = 0.1,
    arrow = arrow(length = unit(0.03, "npc")),
    linewidth = 1,
    color = "black"
...
taxa_levels <- unique(by_species_wtaxa_st %>% filter(dataset == "herb") %>% .$taxa)
group1 <- taxa_levels[1:(length(taxa_levels)/2)]
group2 <- taxa_levels[(length(taxa_levels)/2 + 1):(length(taxa_levels)+1)]
...
plot1 <- by_species_wtaxa_st %>%
  ...
  geom_errorbar(aes(ymin = conf_low_combined, ymax = conf_high_combined),
    ...
    geom_text(aes(y = -22, label = label), vjust = 0, color = "black", size = 8) +
...
by_species_wtaxa_st %>% 
  filter(dataset == "herb") %>% 
  summarise(n = sum(norm_estimate < anom_estimate)/2)/74
...
by_species_wtaxa_st %>% 
  filter(dataset == "herb") %>% 
  summarise(var_norm = var(norm_estimate),
            var_anom = var(anom_estimate),
            mean_norm = mean(norm_estimate),
            mean_anom = mean(anom_estimate))
```
...
by_species_wtaxa_dataset %>% 
  group_by(dataset, sensitivity_type) %>% 
  summarise(mean = mean(estimate_combined),
            sd = sd(estimate_combined
                    ))
```
...
ggplot(combined_df, aes(x = norm_estimate, y = anom_estimate, color = season)) +
  geom_point(
    data = subset(combined_df, season == "spring"),
    aes(x = norm_estimate, y = anom_estimate, color = season),
    alpha = 0.5, size = 2
  ) +
  geom_point(
    data = subset(combined_df, season == "winter"),
    aes(x = norm_estimate, y = anom_estimate, color = season),
    alpha = 0.5, size = 2
  ) +
  geom_text(
    data = subset(combined_df, season == "spring"),
    aes(x = norm_estimate, y = anom_estimate, label = label, color = season),
    vjust = -1, alpha = 0.5, size = 7, show.legend = FALSE
  ) +
  geom_text(
    data = subset(combined_df, season == "winter"),
    aes(x = norm_estimate, y = anom_estimate, label = label, color = season),
...
PMM_winter <- read.csv("data/winter/summary_PMM.csv", row.names = 1)

PMM_winter_st <- read.csv("data/winter/tem_spa/summary_PMM.csv", row.names = 1)

PMM_spring <- read.csv("data/spring/summary_PMM.csv", row.names = 1)

PMM_spring_st <- read.csv("data/spring/tem_spa/summary_PMM.csv", row.names = 1)
```
...
## Plot tree for winter adaptation

```{r}
PMM_species_data_wci <- PMM_winter %>%
  tibble::rownames_to_column("parameter") %>%
  filter(grepl("b_spatial\\[|b_spatial\\[", parameter)) %>%
  mutate(
    param_type = gsub("\\[\\d+\\]", "", parameter),
    group = as.numeric(gsub(".*\\[(\\d+)\\]", "\\1", parameter))
  ) %>%
  dplyr::select(group, param_type, mean, X2.5., X97.5.) %>%
  tidyr::pivot_longer(
...
tree_plot_wa <- ggtree(tree, aes(color = lambda_all), layout = "circular") + 
  geom_tree(size = 1.5) +
  scale_color_gradient2(
    low = "#d7191c",     # Deep red
  ...
  theme_tree() +
    theme(legend.position = c(1.1, 0.04),
              plot.margin = margin(40, 60, 50, 60))  # top, right, bottom, left (in "pt" by default))       # Adjust legend placement

print(tree_plot_wa)
```

## Plot tree for spring plasticity
```{r fig.height=9, fig.width=11}

PMM_species_data_wci <- PMM_spring %>%
  tibble::rownames_to_column("parameter") %>%
  filter(grepl("b_temporal\\[|b_temporal\\[", parameter)) %>%
  mutate(
    param_type = gsub("\\[\\d+\\]", "", parameter),
    group = as.numeric(gsub(".*\\[(\\d+)\\]", "\\1", parameter))
  ) %>%
...
tree_plot_sp <- ggtree(tree, aes(color = lambda_all), layout = "circular") + 
  geom_tree(size = 1.5) +
  scale_color_gradient2(
    low = "#d7191c",     # Deep red
    mid = "#fee08b",     # Brighter yellow
...
## Plot tree for spring adaptation
```{r fig.height=9, fig.width=11}
...
tree_plot_sa <- ggtree(tree, aes(color = lambda_all), layout = "circular") + 
  geom_tree(size = 1.5) +
  scale_color_gradient2(
    low = "#d7191c",     # Deep red
    mid = "#fee08b",     # Brighter yellow
...
## Plot tree for winter plasticity
```{r fig.height=9, fig.width=11}
...
tree_plot_wp <- ggtree(tree, aes(color = lambda_all), layout = "circular") + 
  geom_tree(size = 1.5) +
  scale_color_gradient2(
    low = "#d7191c",     # Deep red
    mid = "#fee08b",     # Brighter yellow
