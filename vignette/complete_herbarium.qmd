...
```
# roughly check whether year/decade has impact on the data
## year: 
```{r}
results <- temperature_data %>%
  group_by(species) %>%
  do(tidy(lm(doy ~ anom + norm + year, data = .))) %>%
  ungroup()

results_table <- results %>%
  select(species, term, estimate, p.value)

# Reshape the data to have species as rows and terms as columns
results_table_wide <- results_table %>%
  pivot_wider(names_from = term, values_from = c(estimate, p.value)) %>% 
  filter(p.value_year < 0.05) 
```
...
results <- temperature_data %>%
  ...
  group_by(species) %>%
  do(tidy(lm(doy ~ anom + norm + decade, data = .))) %>%
  ungroup()
