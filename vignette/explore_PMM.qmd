...
results <- list()
unique_species <- unique(joint_data_total$species)

for (species_name in unique_species) {
  results[[species_name]] <- analyze_species(joint_data_total, species_name)
}

# Combine all summary rows into a single data frame
summary_results <- bind_rows(lapply(results, function(res) res$summary))

write.csv(summary_results, "../data/species_summary_total.csv", row.names = FALSE)
```



```{r}
# try to plot the result and add star but too complex because lack of equal column for now
summary_results <- read.csv("../data/species_summary_total.csv")

```

```{r}
# try to test certain speices

  species_data <- joint_data_total %>% filter(species == "Fraxinus americana")
  ...
  coef_yeart <- model_summary %>% filter(term == "yeart")
