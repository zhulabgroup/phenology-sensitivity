...
```{r}
phylo_species_id <- temperature_data_model %>%
  distinct(species) %>%
  dplyr::mutate(sppid = dplyr::row_number())
...
options(mc.cores = parallel::detectCores())
  ...
  saveRDS(fitlambest, "../data/phylogenetics/fit_model_PMM.rds")
...
```{r}
fitlambest <- readRDS("../data/phylogenetics/fit_model_HMM.rds")
# summarize the results
fitsum <- summary(fitlambest, pars = c("a_z", "sigma_interceptsa", 
...
#                         cov.prior = invwishart(df = 3, scale = diag(c(10, 10, 10), 3)))
# 
