...
records_plot <- ggplot() + #"Field observation" = "#0751f1", "Herbarium" = "#582800"
  geom_col(data = npn_tem, aes(x = year, y = count), fill = "#0751f1", alpha = 0.5) +
  geom_col(data = herb_tem, aes(x = year, y = count), fill = "#582800", alpha = 0.5) +
  labs(
    x = "Year",
    y = "Number of\nrecords"
  ) 
...
map_plot <- ggplot() +
  ...
  ) + #"Field observation" = "#0751f1", "Herbarium" = "#582800"
  scale_color_manual(values = c("Field observation" = "#0751f1", "Herbarium" = "#582800")) +
  labs(
    color = "Data source",
    x = "Longitude",
    y = "Latitude",
...
# Figure 2 compare two dataset sensitivity
...
# Figure 3: the by group result from the two datasets
## herbairum
prepare data
```{r}
...
# Supplementary Figure 4: sensitivity consistency among species
...
npn_data <- read.csv("../data/species_summary_npn.csv") %>% 
  mutate(dataset = "Field observation") 
species_list <- full_join(herbarium_data, npn_data, by = "species") %>%
  dplyr::select(species) %>%
  distinct()
```
...
joint_data <- rbind(npn_data, herb_data) %>%
  filter(species %in% species_list$species) %>%
  mutate(dataset = factor(dataset, levels = c("Field observation", "Herbarium"))) |> 
  inner_join(taxa_info) # only speceis included in the HMM and PMM model
...
# Create the plot
ggplot(joint_data, aes(x = species, fill = dataset)) +
  geom_bar(position = "dodge", stat = "count") +  # Adjust bar width
  scale_y_log10() +
  facet_grid(. ~ taxa, scales = "free_x", space = "free_x") + 
  scale_fill_manual(values = c("Field observation" = "#0751f1", "Herbarium" = "#582800")) +  # Specify custom colors
  labs(
    x = "Species",
    y = "Number of records",
    fill = "Dataset"
