...
theme(
  ...
  plot.title = element_text(size = 25, hjust = -0.05) # Adjust title size here
) +
...
figure_1b <- ggplot(data = black) + 
  ...
  xlim(-0.3, 1) +
  ylim(-0.1, 1) +
labs(x = expression("Temperature anomaly" ~ t[anom]), y = expression("Flowering day anomaly" ~ d[anom]), title = "b.") +
  theme(axis.title.x = element_text(size = 18), axis.title.y = element_text(size = 18), plot.title = element_text(size = 25, hjust = -0.05) # Adjust title size here
) +
  annotate("text", x = -0.12 , y = 0.02,, label = "Early flowering", size = 6, hjust = 0.7) +
  annotate("text",  x = -0.12 , y = 1, label = "Late flowering", size = 6, hjust = 0.7) +
...
figure_1c <- ggplot(mapping = aes(x = x, y = y)) + 
  ...
  labs(x = expression(t[observe]), y = expression(d[observe]), title = "c.") + 
  theme_classic() + 
  theme(axis.text.x = element_blank(), 
        axis.text.y = element_blank(),
        axis.ticks = element_blank(),
        ...
        axis.title.x = element_text(size = 18), axis.title.y = element_text(size = 18),
          plot.title = element_text(size = 25, hjust = -0.025) # Adjust title size here
)
```
...
# Combine the plots
final_plot <- (figure_1a + figure_1b) / figure_1c  
