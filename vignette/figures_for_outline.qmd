...
double_y_plot <- ggplot() +
  # First dataset: Temperature anomaly
  ...
  # Second dataset: Number of records
  geom_col(data = npn_tem, aes(x = year, y = n / 500), fill = "forestgreen", alpha = 0.5) + # Scale n down for alignment
  geom_col(data = herb_tem, aes(x = year, y = n / 500), fill = "chocolate4", alpha = 0.5) +
  # Primary and secondary y-axes
  scale_y_continuous(
    name = "Temperature\nanomaly (°C)", # Primary axis label
    sec.axis = sec_axis(~ . * 500, name = "Number of records") # Secondary axis label and scaling
  ) +
  labs(x = "Year") +
  theme(
    axis.text.y = element_text(color = "purple"), # Primary axis text color
    axis.title.y = element_text(color = "purple"), # Primary axis title color
...
layout <- c(
  ...
  area(t=2,l=6,b=3,r=6),
  area(t=1,l=7,b=1,r=8),
  area(t=2,l=7,b=2,r=8),
  area(t=3,l=7,b=3,r=8)
)
...
# Adjust the margins and layout
combined_plot <- double_y_plot / map_plot +
                 normality_plot +
...
# Adjust the margins and layout
combined_plot_2 <- double_y_plot / combined_plot 
...
# Combine the plots
final_plot <- figure_1b / figure_1a / figure_1c 
# Print the combined plot
print(final_plot)
```
