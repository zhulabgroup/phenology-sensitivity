...
# Research questions:
1. What's the flower-leaf sequence across different genus and species in INDIVIDUAL level?
2. How does climatic factors influence flower-leaf sequence? (order, lag)
3. Can we use leaf phenology and flower-leaf sequence to predict pollen phenology?

# 1. Flower-leaf sequence in INDIVIDUAL level

## 1.1 flower-leaf sequence across different genus
Starts with the 11 allergy causing genus.
...
## 1.2 flower-leaf sequence across different species
...
## 1.3 Case study: oak species and acer species
...
```{r figure3v2:temporal, message=FALSE, warning=FALSE, fig.width=6*1.618 , fig.height=6, cache=TRUE}
# to get p-value, go back to linear model with intercept (0 point at the center may affet the intercept, so delete if only 1 observation)
source("scripts/plot_individual_oak_norm_v2.R")
plot_individual_varibility
#rm(list = ls())
```
