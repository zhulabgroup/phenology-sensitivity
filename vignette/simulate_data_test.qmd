...
## check whether blme works for group with 1 observation only
```{r}
# blme doesn'r work if each group only has 1 observatin, lets add one more to one of the group
additional_dots <- additional_dots %>% 
  bind_rows(data.frame(norm = 1, y_i = 8.513217, anom_i = 0, label = "around_center_1"))
library(blme)
