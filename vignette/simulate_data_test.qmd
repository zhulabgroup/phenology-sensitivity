...
# fake data for herbarium
when fake the data, we can decide whether to add noice, and how much noise to add
```{r}
# Load necessary package
...
for (i in 1:length(x_center)) {
  ...
  dots <- data.frame(norm = x_center[i], y = y_i, anom = anom_i, label = paste("around_center", i, sep = "_"))
  additional_dots <- rbind(additional_dots, dots)
}
for (i in 1:length(x_center)) {
  anom_i <- rnorm(1, 0, 0.2)  # Randomize x-coordinates
  y_i <- 10 - x_center[i]-2*anom_i +  rnorm(1,0,0.2)
  dots <- data.frame(norm = x_center[i], y = y_i, anom = anom_i, label = paste("around_center", i, sep = "_"))
  additional_dots <- rbind(additional_dots, dots)
}
...
# Plot the results
sepp <- ggplot(center_dots, aes(x = norm+anom, y = y)) +
  geom_point() +
  ...
  theme_minimal() +
  theme(legend.position = "none") + # Legend position
  geom_point(data = additional_dots, aes(x = norm+anom, y = y), color = "red")
print(sepp)
```

```{r}
model <- lm(y~norm+anom,additional_dots)
summary(model)
confint(model)
```
