...
# ln -s /nfs/turbo/seas-zhukai/proj-phenology-sensitivity data
# ln -s /Volumes/seas-zhukai/proj-phenology-sensitivity data
...
## simple linear regression with stacked temporal and aggregated spatial
```{r}
...
```{r}
source("scripts/table_spatial_temporal_statistics.R")
species_summary
...
lme_result <- lmerTest::lmer(leaf ~ spring_avg_temp + 
                                 ...
                                 data = anomaly_data)
summary(lme_result)
summary(lme_result)$coefficients["spring_avg_temp_anomaly", "Estimate"]
```
...
## bayesian for spatial temporal comparasion
```{r}
source("function_bayesmodel_temporal.R")
source("function_bayesmodel_spatial.R")
source("pdf_spatem_posterior.R")
source("pdf_neontem_posterior.R")
...
```{r}
source("plot_summary_posteriors.R")
